...
severity_stats = function(polyint, firehist, reference.date, lagyears){
  ...
  tlist <- tlist %>% 
    mutate(fnum = stringr::str_split_fixed(name, "_", 7)[,3]) %>%
...
sev.out = sev.test.out.df %>% 
  ...
  dplyr::select(TRANSECT, TRAP_START, prop_burnt_transect, prop_moderate_transect, 
...
for (i in 1:length(trap.dates)) {
  ...
  harvestage.diversity = landscapemetrics::sample_lsm(as.integer(tsh_raster),
                                                      y=transect_buffer_550m, what='lsm_l_shdi')
...
## Rainfall
```{r}
rain.rasters = list.files("~/Dropbox/Billy/_Research/data/spatial/climate grids/rainfall", pattern=".grid", full.names = TRUE)
...
## Productivity
```{r}
prod.rasters = intersect(list.files("~/Dropbox/Billy/_Research/_PhD/07_UpperWarren_WA/data/NDVI", pattern=".tif$", full.names = TRUE), list.files("~/Dropbox/Billy/_Research/_PhD/07_UpperWarren_WA/data/NDVI", pattern="MOD13A3.006__1_km_monthly_NDVI", full.names = TRUE))
...
## Pasture
```{r}
load("~/Dropbox/Billy/_research/_PhD/07_UpperWarren_WA/data/si50_woody_geo/pasture.stack.30052021.RData")
...
## Planned Burn Assets
```{r}
assets = read_sf("~/Dropbox/Billy/_research/_PhD/07_UpperWarren_WA/data/from Adrian Feb19/Spatial data/Covariates/Assets_PlannedBurns.shp")
...
## Road Density
```{r}
roads = read_sf("~/Dropbox/Billy/_research/_PhD/07_UpperWarren_WA/data/from Adrian Feb19/Spatial data/Covariates/TGDB_Roads_Eradicat.shp")
...
```{r}
annual.prec = raster("~/Dropbox/Billy/_Research/_PhD/07_UpperWarren_WA/data/Climate Data/rainan 4.txt")
...
```{r}
veg.complex =  read_sf("~/Dropbox/Billy/_Research/_PhD/07_UpperWarren_WA/data/from Adrian Feb19/Spatial data/Covariates/Vegetation_Complexes_WarrenJarrah.shp")
...
write.csv(covariates.table, "~/Dropbox/Billy/_research/_PhD/07_UpperWarren_WA/_Projects/UpperWarrenSEM/Data_Clean/transect.sem.data.Dec2022.csv")
