...
```{r}
project.area = read_sf("~/Dropbox/_Research/_PhD/07_UpperWarren_WA/data/from Adrian Feb19/Spatial data/Boundaries/WarrenJarrah_projectboundary.shp")
...
```{r}
trap.points = read.csv("~/Dropbox/_research/_PhD/07_UpperWarren_WA/_Projects/SSM_SEM/Data_Processing/trap.data.csv")
...
for (i in 1:length(trap.dates)) {
  ...
  fireage.cat.diversity = landscapemetrics::sample_lsm(as.integer(tsf_category_raster), y=transect_buffer_500m, what='lsm_l_shdi')
...
firehist <- st_read("~/Dropbox/_Research/_PhD/07_UpperWarren_WA/data/from Adrian Feb19/Fire Severity prep/For Analysis/firehistory_upperwarren.shp")%>%
  st_transform(mga50)
...
severity_stats = function(polyint, firehist, reference.date, lagyears){
  ...
  tlist <- data.frame(name = list.files("~/Dropbox/Upper Warren Fire Severity/CBI_maps", pattern = "tif"),
                      path = list.files("~/Dropbox/Upper Warren Fire Severity/CBI_maps", pattern = "tif", full=TRUE))
  tlist <- tlist %>% 
...
harvest.history = read_sf("~/Dropbox/_Research/_PhD/07_UpperWarren_WA/data/from Adrian Feb19/Spatial data/Covariates/SilviculturalOperations_WarrenJarrah.shp")
...
## Topography
https://ecat.ga.gov.au/geonetwork/srv/eng/catalog.search#/metadata/123119
...
depression and swamps are crap
midslopes are the good stuff
uplands can be a bit grainy, 
quenda depressions
chuditch whatever
