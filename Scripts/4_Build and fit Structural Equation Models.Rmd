...
trap.sessions = trap.sessions %>% mutate(TRANSECT = factor(TRANSECT, levels = sort(unique(trap.sessions$TRANSECT), decreasing = TRUE)))
...
quenda.m <- bf(Quenda_Caps | trials(Effort) ~ 1 , family='binomial')
### Fire
...
quenda.m0 <- bf(Quenda_Caps | trials(Effort)  ~ 1 + (1|Phase), family='binomial')
### Fire
...
quenda.m1 <- bf(Quenda_Caps | trials(Effort) ~ 
                  scale(BaitIntensity.Transect.Twelve) + scale(mean.smNDVI) + scale(ndvi.monthly.anomaly) + 
                            scale(prop.pasture.1km) + scale(DistCentroidPasture) +  scale(valleys.prop.area) +
                            scale(prop_moderate_transect) + scale(tsf.cat.shannon.diversity) + 
                            scale(prop.burnttwice.500m.transect) + scale(prop_severe_transect) + 
                            scale(tsh.shannon.diversity) +
...
quenda.m2 <- bf(Quenda_Caps | trials(Effort) ~ 
                  scale(BaitIntensity.Transect.Twelve) + scale(mean.smNDVI) + scale(ndvi.monthly.anomaly) + 
                            scale(prop.pasture.1km) + scale(DistCentroidPasture) +  scale(valleys.prop.area) +
                            scale(prop_moderate_transect) + scale(tsf.cat.shannon.diversity) + 
                            scale(prop.burnttwice.500m.transect) + scale(prop_severe_transect) + 
...
quenda.m3 <- bf(Quenda_Caps | trials(Effort) ~ 
                  scale(BaitIntensity.Transect.Twelve) + scale(RoadLength1km) + scale(ndvi.monthly.anomaly) + 
                            scale(prop.pasture.1km) + scale(DistCentroidPasture) +  scale(valleys.prop.area) +
                            scale(prop_moderate_transect) + scale(tsf.cat.shannon.diversity) + scale(prop.burnttwice.500m.transect) + 
                            scale(tsh.shannon.diversity) +
                            scale(Chuditch_Abundance) + (1|Phase), family='binomial')
...
quenda.m4 <- bf(Quenda_Caps | trials(Effort)  ~ 
                            scale(BaitIntensity.Transect.Twelve) + scale(RoadLength1km) + scale(ndvi.monthly.anomaly) + 
                            scale(prop.pasture.1km) + scale(DistCentroidPasture) +  scale(valleys.prop.area) +
                            scale(prop_moderate_transect) + scale(tsf.cat.shannon.diversity) + scale(prop.burnttwice.500m.transect) + 
                            scale(prop.20yrsharvest.500m.transect) +
...
save(best_sem, file="~/Dropbox/Billy/_research/_PhD/07_UpperWarren_WA/_Projects/UpperWarrenSEM/Data_Clean/best.sem.fit.Dec2022.Rdata")
...
quenda.vars =  c(sem.vars[grep("b_QuendaCaps_scale",sem.vars)], sem.vars[grep("bsp_QuendaCaps",sem.vars)])
...
var.lookup = var.lookup %>% 
  ...
  mutate(.variable = gsub("b_QuendaCaps_scale","",.variable)) %>%
  mutate(.variable = gsub("bsp_QuendaCaps_","",.variable)) %>%
...
#write.csv(coef.summary, "~/Dropbox/Billy/_research/_PhD/07_UpperWarren_WA/_Projects/UpperWarrenSEM/Outputs/sem.coef.summary.Dec2022.csv")
